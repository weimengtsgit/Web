...
public class HibernateUtils {
	...
	private static Criterion createCriterion(String fieldName,
		...
		case NOTIN: // >=
			...
			break;
		case NOTLIKE: // >=
			criterion = Restrictions.not(Restrictions.like(fieldName, (String) fieldValue,
					MatchMode.ANYWHERE));
			break;
		case NOTEQ: // >=
			criterion = Restrictions.not(Restrictions.eq(fieldName, fieldValue));
			break;
		}
		return criterion;
	}
...
//	}
	...
	public static Object uniqueResult(Session session, String sql, Object... values) {
		Assert.hasText(sql, "sql不能为空");
		SQLQuery query = session.createSQLQuery(sql);
		for (int i = 0; i < values.length; i++) {
			query.setParameter(i, values[i]);
