...
<form rel="pagerForm" id="pagerForm" method="post" action="${ctx}/customer/statShow/lookupList" onsubmit="return dwzSearch(this, 'dialog');">
...
<div class="pageHeader">
	...
	<div class="searchBar">
		<ul class="searchContent">
				...
				<li>
					<label>访问来源：</label>
					<tag:enum id="accessType" defaultValue="" name="filter_EQS_accessType" enumName="com_statShow_accessType" enumKey="${pageData.filtraMap.EQS_accessType}" />
				</li>
		...
		<div class="subBar">
			<ul>
				...
				<%-- <li><a class="button" href="${ctx}/customer/statShow/advancedSearch" target="dialog" title="查询框"><span>高级查询</span></a></li> --%>
			</ul>
		</div>
	</div>
	</div>
		...
		<tbody>
			<c:forEach var="entity" items="${pageData.result}">
				<tr>
					...
					<td><tag:enum isTag="false" id="accessType" name="accessType" enumName="com_statShow_accessType" enumKey="${entity.accessType}" /></td>		
					...
					<td>
						<a class="btnSelect" href="javascript:SLAVEJS.getLoopupJson('${ctx}/customer/statShow/viewJson?id=${entity.id}')" title="查找带回">选择</a>
					</td>
				</tr>
			</c:forEach>
		</tbody>
