...
<form id="pagerForm" onsubmit="return dwzSearch(this, 'dialog');" method="post" action="${ctx}/mall/shop/viewRefList">
	...
	<div class="searchBar">
		<ul class="searchContent">
				...
				<li>
					...
					<input class="hiddenItem"  type="hidden" name="filter_EQI_device.id" value='${pageData.filtraMap["EQI_device.id"]}' />
		...
		<tbody>
			<c:forEach var="entity" items="${pageData.result}" varStatus="s">
				<tr target="dialog" mask="true" >
					...
					<a class="ghbutton search right" href="${ctx}/view?id=${entity.mall.id}" target="dialog" mask="true" rel="mall_shop_lookupdialog" title="查看商场详情">${entity.mall.name}</a>
					</c:if>
					</td>
