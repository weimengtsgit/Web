...
<form id="pagerForm" onsubmit="return dwzSearch(this, 'dialog');" method="post" action="${ctx}/mall/shopPromotion/viewRefList">
		...
		<tbody>
			<c:forEach var="entity" items="${pageData.result}" varStatus="s">
				<tr target="dialog" mask="true" >
					...
					<a class="ghbutton search right" href="${ctx}/view?id=${entity.shop.id}" target="dialog" mask="true" rel="mall_shopPromotion_lookupdialog" title="查看商铺详情">${entity.shop.name}</a>
					</c:if>
					</td>
					...
					<td><tag:enum isTag="false" tagType="view" id="auditStatus" name="auditStatus" enumName="shopAuditStatus" enumKey="${entity.auditStatus}" /></td>
