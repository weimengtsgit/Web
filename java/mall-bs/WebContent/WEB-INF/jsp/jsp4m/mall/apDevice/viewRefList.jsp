...
<form id="pagerForm" onsubmit="return dwzSearch(this, 'dialog');" method="post" action="${ctx}/mall/apDevice/viewRefList">
		...
		<tbody>
			<c:forEach var="entity" items="${pageData.result}" varStatus="s">
				<tr target="dialog" mask="true" >
					...
					<a class="ghbutton search right" href="${ctx}/mall/mall/view?id=${entity.mall.id}" target="dialog" mask="true" rel="mall_apDevice_lookupdialog" title="查看商场详情">${entity.mall.name}</a>
					</c:if>
					</td>
					...
					<td><tag:enum isTag="false" tagType="view" id="status" name="status" enumName="deviceStatus" enumKey="${entity.status}" /></td>
