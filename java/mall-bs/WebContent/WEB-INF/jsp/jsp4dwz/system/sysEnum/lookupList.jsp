...
<form rel="pagerForm" id="pagerForm" method="post" action="${ctx}/system/sysEnum/lookupList" onsubmit="return dwzSearch(this, 'dialog');">
	...
	<div class="searchBar">
		...
		<div class="subBar">
			<ul>
				...
				<%-- <li><a class="button" href="${ctx}/system/sysEnum/advancedSearch" target="dialog" title="查询框"><span>高级查询</span></a></li> --%>
			</ul>
		</div>
	</div>
	</div>
		...
		<tbody>
			<c:forEach var="entity" items="${pageData.result}">
				<tr>
					...
					<td>
						<a class="btnSelect" href="javascript:SLAVEJS.getLoopupJson('${ctx}/system/sysEnum/viewJson?id=${entity.id}')" title="查找带回">选择</a>
					</td>
				</tr>
			</c:forEach>
		</tbody>
