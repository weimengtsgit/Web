	...
	$.extend($.fn, {
		jPanel:function(options){
			...
			return this.each(function(){
				...
				var hchartElement = $($pucker.context).find('.hchart');
				var hchartId = $(hchartElement).attr('id');
				if(hchartId){
					$panel.dblclick(function(){
						var h1Element = $($pucker.context).find('h1');
						var hchartTitle = $(h1Element).html() || "";
						
						var options = {};
						options.width = $(window).width();
						options.height = $(window).height();
						options.mask = "dwzhightcharts";
						options.minable = "";
						options.maxable = "";
						
						options.hchartId = hchartId;
						$.pdialog.open("dwzhightcharts", "dwzhightcharts", hchartTitle, options);
						return false;
					});
				}
			});
