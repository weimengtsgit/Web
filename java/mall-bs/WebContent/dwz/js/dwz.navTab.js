...
var navTab = {
	...
	_init: function(){
		...
		this._getTabs().each(function(iTabIndex){
			...
			$(this).unbind("dblclick").dblclick(function(event){
			...
			if (url) {
				if ($tab.hasClass("external")) {
					...
					if ($.isEmptyObject(op.data)) { //获取pagerForm参数
						...
						if($(op.data).size() > 0 && url.indexOf("?") != -1){
							//获得url参数
							var uri = url.substring(0, url.indexOf("?"));
							var paraString = url.substring(url.indexOf("?") + 1, url.length).split("&");
							var paraObj = {}
							for (var i = 0; i < $(paraString).size(); i++) {
					        	var j = paraString[i];
					        	paraObj[j.substring(0, j.indexOf("="))] = j.substring(j.indexOf("=") + 1, j.length);
					        }
							
							jQuery.each(paraObj, function(pn, pv){
								var flag = true;
								jQuery.each( op.data, function(i, field){
