...
(function($){
	...
	$.extend({
		getParent:function(){
			return  _lookup['$target'].parents(".unitBox:first");
		},
			...
			$box.find(":input").each(function(){
				...
				for (var key in args) {
					...
					if(name.indexOf("[") != -1 && name.indexOf("]") != -1){
						var inputNamec = inputName.substring(0,inputName.indexOf("["))+inputName.substring(inputName.indexOf("]"));
						var namec = name.substring(0,name.indexOf("["))+name.substring(name.indexOf("]"))
					...
					if(inputName){
						var fnames = inputName.split('_');
						if(fnames&&fnames.length==3&&fnames[0]=='filter'&&fnames[2]==name){
							$input.val(args[key]);
							break;
						}
					}
		...
		bringBackSuggest4attachment: function(args){
			var $box = _lookup['$target'].parents(".unitBox:first");
			$box.find(":input").each(function(){
				var $input = $(this), inputName = $input.attr("name");
			...
			$box.find(":input").each(function(){
				var $input = $(this), inputName = $input.attr("backname");
			...
			$box.find("._flush").each(function(){
				var $input = $(this), inputName = $input.attr("name"),flush=$input.attr("flush"),uri=$input.attr("uri");
				
				for (var key in args) {
					var name = (_lookup.pk == key) ? _util.lookupPk(key) : _util.lookupField(key);
					if (name == inputName) {
						$input.attr(flush,uri+args[key]);
						break;
					}
				}
			});
		...
		lookup: function(){
			return this.each(function(){
				var $this = $(this), options = {mask:true, 
					width:$this.attr('width')||800, height:$this.attr('height')||460,
		...
		itemDetail: function(){
			...
			function tdHtml(field){
				...
				switch(field.type){
					...
					case 'file':
						html = '<input type="hidden" name="'+field.lookupGroup+'.'+field.lookupPk+suffix+'"/>'
							+ '<input type="file" name="'+field.name+'" size="'+field.size+'" readonly="readonly" class="'+field.fieldClass+'"/>';
						break;
					case 'enum':
						$.ajax({
