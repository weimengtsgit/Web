...
function initLayout(){
	...
	/*添加内容 start*/
	if(DWZ.ui.barup){//向上
		$("#container .tabsPageContent").height($(window).height() - 34 -35).find("[layoutH]").layoutH();
		$("#sidebar").height($(window).height() - 5 -35);
		
		if(!DWZ.ui.sbar){//不隐藏
			$("#sidebar_s .collapse").height($(window).height() - 5 -35);
		}
	}else{
		$("#container .tabsPageContent").height(iContentH - 34).find("[layoutH]").layoutH();
		$("#sidebar").height(iContentH - 5);
		if(!DWZ.ui.sbar){//不隐藏
			$("#sidebar_s .collapse").height(iContentH - 5);
		}
	}
	$("#splitBar, #splitBarProxy").height(iContentH - 5);
...
function initUI(_box){
	...
	$("tr[target=dialog]", $p).each(function(){
		...
		$(this).dblclick(function(event){
			...
			var $this = $(this);
				...
				if(!href){
					href =  $this.find('a[class=btnView]').attr('href');
				}
				if(!title){
					title =  $this.find('a[class=btnView]').attr('title');
				}
				var url = unescape(href).replaceTmById($(event.target).parents(".unitBox:first"));
				DWZ.debug(url);
				if (!url.isFinishedTm()) {
	...
	// 这里放其他第三方jQuery插件...
	if ($.fn.limit) $(".pageContent tbody *[limit]", $p).limit();
	if ($.fn.dwzhighcharts) $(".hchart", $p).dwzhighcharts();
}
