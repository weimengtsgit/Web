...
(function($){
	$.fn.jTable = function(options){
		return this.each(function(){
			...
			var curScrollerFlag = $(tbody).attr('scroller');//滚动条是否显示标示符,默认显示,no:不显示
			var mintrNum = $(tbody).attr('mintr') || 19;//最小tr数目,默认19
			var layoutStr = layoutH ? " layoutH='" + layoutH + "'" : "";
			
			if(curScrollerFlag=='no'){
				tbody.wrap("<div scroller='no' class='gridScroller'" + layoutStr + " style='width:" + $tc.width() + "px;'><div class='gridTbody'><table style='width:" + (tlength - 20) + "px;'></table></div></div>");
				
				/*添加默认显示行数-->div充当tr总高度*/
				var curMintrNum = tbody.find('>tr').size();
				if(curMintrNum < mintrNum){
					var curHeight = (mintrNum - curMintrNum) * 22;
					$grid.append("<div style='width: 100%;height: "+curHeight+"px;'></div>");
				}
