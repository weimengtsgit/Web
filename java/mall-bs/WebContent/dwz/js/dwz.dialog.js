...
(function($){
	$.pdialog = {
		_op:{height:460, width:860, minH:40, minW:50, total:20, max:false, mask:false, resizable:true, drawable:true, maxable:true,minable:true,fresh:true},
		_current:null,
		_zIndex:42,
		getCurrent:function(){
		...
		open:function(url, dlgid, title, options) {
			...
			if(dialog) {
				...
				if(op.fresh || url != $(dialog).data("url")){
					...
					var jDContent = dialog.find(".dialogContent");
					if(url == 'dwzhightcharts'){
						var hchartId = op.hchartId;
						eval('jDContent.highcharts('+hchartId+');');
						$.taskBar.addDialog(dlgid,title);
						
						$(jDContent).dblclick(function(event){
							$.pdialog.close(dialog);
							return false;
						});
					}else{
...
//					$.pdialog.switchDialog(dialog);
			...
			// 强制弹出带taskBar的窗口，并强制加上遮罩层
			if (op.mask&&false) {
				$(dialog).css("zIndex", 1000);
				$("a.minimize",dialog).hide();
				$(dialog).data("mask", true);
				$("#dialogBackground").show();
				$("#dialogBackground").dialog=$(dialog);
		...
		close:function(dialog) {
			...
			if(false){
				...
				//alert($.taskBar._getTasks().size());
				// 当taskBar没有弹出窗口时隐藏遮罩层
				if($.taskBar._getTasks().size()<=0){
