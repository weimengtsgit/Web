...
(function($){
	...
	$.fn.jBar = function(options){
		var op = $.extend({container:"#container", collapse:".collapse", toggleBut:".toggleCollapse div", sideBar:"#sidebar", sideBar2:"#sidebar_s", splitBar:"#splitBar", splitBar2:"#splitBarProxy", upBut:"#navMenu .barup", header:"#header .headerNav", footer:"#footer",leftside:"#leftside"}, options);
		return this.each(function(){
			var jbar = this;
			var sbar = $(op.sideBar2, jbar);
			var bar = $(op.sideBar, jbar);
			var header = $(op.header);
			var footer = $(op.footer);
			...
			$(op.upBut).click(function(){//向上移动
				if(DWZ.ui.barup){//当前为向上
					DWZ.ui.barup = false;
					$(op.leftside).animate({top: 85}, 100, function(){
						$(op.container).css({top:85});
						header.show();
						footer.show();
						initLayout();
						$(window).trigger(DWZ.eventType.resizeGrid);
					...
					header.hide();
					footer.hide();
					$(op.container).animate({top:35},100,function(){
						$(op.leftside).animate({top: 35}, 10, function(){
								initLayout();
								$(window).trigger(DWZ.eventType.resizeGrid);
						});
					});
